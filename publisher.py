...
def _publish_new_packages(repo, to_add, pub, dist, gpg_conf, comp):
    ...
    subprocess.run([APTLY_COMMAND, 'repo', 'add', '-remove-files', repo, to_add
                   ], check=True)
    ...
    if pub not in existing_pubs:
        subprocess.run(
            [APTLY_COMMAND, 'publish', 'repo', '-batch',
             f'-component="{comp}"', *gpg_flags, repo],
            check=True
        )
    else:
        subprocess.run(
            [APTLY_COMMAND, 'publish', 'update',
             '-batch', *gpg_flags, repo, pub],
            check=True
        )
